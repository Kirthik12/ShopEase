...
public static class DemoDataSeeder
{
    ...
    private static async Task<List<User>> SeedUsersAsync(ShopEaseDbContext db)
    {
        ...
        foreach (var s in seed)
        {
            var user = new User
            {
                ...
                CreatedAt = DateTime.Parse(s.Created),
            };
    ...
    private static async Task<List<Category>> SeedCategoriesAsync(ShopEaseDbContext db)
    {
        ...
        var categories = seed.Select(s => new Category { Name = s.Name, Description = s.Description, IsActive = true, CreatedAt = DateTime.Parse("2026-01-01") }).ToList();
    ...
    private static async Task<List<Product>> SeedProductsAsync(ShopEaseDbContext db, List<Category> categories)
    {
        ...
        var products = seed.Select(s => new Product
        {
            ...
            CategoryId = categories[s.CategoryIndex].Id, Description = s.Description, IsActive = true, CreatedAt = DateTime.Parse(s.Created),
        }).ToList();
    ...
    {
        Dictionary<int, string> methodNames = new() { [1] = "Credit Card", [2] = "UPI", [3] = "Cash on Delivery" };
