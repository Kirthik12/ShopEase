...
public class BackupJobRepository : IBackupJobRepository
{
    ...
    public Task<List<BackupJob>> GetAllAsync() => _db.BackupJobs.AsNoTracking().OrderBy(j => j.Name).ToListAsync();
    ...
    public Task<List<BackupJob>> GetActiveScheduledAsync() =>
        _db.BackupJobs.AsNoTracking().Where(j => j.Active && j.Schedule != "Manual").ToListAsync();
